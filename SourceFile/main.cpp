...
int main() {
  ...
  for (float y = -9.0f; y < 9.0f; y += 0.1) {
    for (float x = -16.0f; x < 16.0f; x += 0.1) {
      layer.add(new Sprite(x, y, 0.09f, 0.09f,
  ...
  while (!window.closed()) {
    ...
    shader.enable();
    // shader.setUniform2f("light_pos", vec2((float)(x * 32.0f / 960.0f
    // - 16.0f), (float)(9.0f - y * 18.0f / 540.0f)));
    shader.setUniform2f("light_pos", vec2(-8, -3));
    shader2.enable();
    shader2.setUniform2f("light_pos", vec2((float)(x * 32.0f / 960.0f - 16.0f),
